...
FixNA.default <- function(X, k = 1:12, eps = 1e-06, maxiter = 1000, G = c("pow", "lcosh"),
  ...
  while (crit > eps) {
    if (G == "pow") {
      for (i in 1:nk) {
        Tk[ , , i] <- .Call( "TIK", Y, U, k[i], method = met, PACKAGE = "tsBSS")$Tik
      }
    } else {
      for (i in 1:nk) {
        Tk[ , , i] <- .Call( "TIKlc", Y, U, k[i], method = met, PACKAGE = "tsBSS")$Tik
      }
    }
